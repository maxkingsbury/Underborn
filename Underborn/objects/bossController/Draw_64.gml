...
draw_text(display_get_gui_width() / 2, display_get_gui_height() * 0.04 + 35, time_text);
...
draw_text(bar_x + bar_width / 2, bar_y+21, "Lv. " + string(oPlayer.level));
...
for (var i = 0; i < 6; i++) {
    ...
    if (weapon_name != undefined) {
        ...
        draw_set_color(c_white);
		if (weapon_level >= 7) {
			draw_text(icon_x, icon_y + 35, "MAX");
		} else {
			draw_text(icon_x, icon_y + 35, "Lv. " + string(weapon_level));
		}
    }
}
...
draw_set_color(c_white);
draw_text(x1 + icon_size + margin, y_pos+15, string(global.enemiesKilled));
...
// Draw the coin count
draw_text(x2 + icon_size + margin, y_pos+15, string(global.coinsThisRound));
